...
public :: atmosphere_init, atmosphere, atmosphere_end, atmosphere_domain
   ...
   type(interpolate_type),save         ::  t_interp
   type(interpolate_type),save         ::  tau_interp

!=================================================================================================================================
logical :: idealized_moist_model = .false.
logical :: nudging = .false.
logical :: nudge_temp = .false.
logical :: nudge_winds = .false.

character(len=256) :: u_wind_file='u'  ! Name of file relative to $work/INPUT  Used only when equilibrium_t_option='from_file'
character(len=256) :: v_wind_file='v'  ! Name of file relative to $work/INPUT  Used only when equilibrium_t_option='from_file'
character(len=256) :: t_file='t'  ! Name of file relative to $work/INPUT  Used only when equilibrium_t_option='from_file'
character(len=256) :: tau_file='tau'  ! Name of file relative to $work/INPUT  Used only when equilibrium_tau_option='from_file' or 'strat_file'

namelist/atmosphere_nml/ idealized_moist_model, nudging, nudge_winds, nudge_temp, tau_file, u_wind_file, v_wind_file, t_file
...
real, allocatable, dimension(:,:,: ) ::  u_nudging, dt_ug_nudging, tau_nudging
real, allocatable, dimension(:,:,: ) ::  v_nudging, dt_vg_nudging
real, allocatable, dimension(:,:,: ) ::  t_nudging, dt_tg_nudging
...
integer :: seconds, days, lon_max, lat_max, ntr, nt, i, j, k, nml_unit, io, stdlog_unit
...
allocate (u_nudging      (is:ie, js:je, num_levels))
allocate (dt_ug_nudging      (is:ie, js:je, num_levels))
allocate (v_nudging      (is:ie, js:je, num_levels))
allocate (dt_vg_nudging      (is:ie, js:je, num_levels))
allocate (t_nudging      (is:ie, js:je, num_levels))
allocate (dt_tg_nudging      (is:ie, js:je, num_levels))
allocate (tau_nudging      (is:ie, js:je, num_levels))
...
if(nudging) then
  ...
  call interpolator_init(tau_interp,    trim(tau_file)//'.nc', rad_lonb_2d, rad_latb_2d, data_out_of_bounds=(/CONSTANT/))

  if(nudge_temp) then
    call interpolator_init(t_interp,    trim(t_file)//'.nc', rad_lonb_2d, rad_latb_2d, data_out_of_bounds=(/CONSTANT/))
  endif

  if(nudge_winds) then
    call interpolator_init(v_interp,    trim(v_wind_file)//'.nc', rad_lonb_2d, rad_latb_2d, data_out_of_bounds=(/CONSTANT/))
    call interpolator_init(u_interp,    trim(u_wind_file)//'.nc', rad_lonb_2d, rad_latb_2d, data_out_of_bounds=(/CONSTANT/))  
  endif
...
if(nudging) then
  ...
  call get_tau(Time, p_half(:,:,:,current), tau_nudging)

  if(nudge_temp) then
    call get_temp(Time, p_half(:,:,:,current), t_nudging) 
    dt_tg_nudging(:,:,:) = -1./(tau_nudging(:,:,:)*86400)*(tg(:,:,:,current)-t_nudging(:,:,:))
    dt_tg(:,:,:) = dt_tg(:,:,:) + dt_tg_nudging(:,:,:)
  endif

  if(nudge_winds) then
    call get_wind(Time, p_half(:,:,:,current), u_nudging, v_nudging)
!    dt_ug_nudging(:,:,:) = -1./(tau_nudging(:,:,:)*86400)*(ug(:,:,:,current)-u_nudging(:,:,:))
!    dt_vg_nudging(:,:,:) = -1./(tau_nudging(:,:,:)*86400)*(vg(:,:,:,current)-v_nudging(:,:,:))

!zonal mean nudging
do l=0, size(ug,1)
    dt_ug_nudging(l,:,:) = -1./(tau_nudging(l,:,:)*86400)*(sum(ug(:,:,:,current),DIM=1)/size(ug,1) - sum(u_nudging(:,:,:),DIM=1)/size(ug,1))
    dt_vg_nudging(l,:,:) = -1./(tau_nudging(l,:,:)*86400)*(sum(vg(:,:,:,current),DIM=1)/size(vg,1) - sum(v_nudging(:,:,:),DIM=1)/size(vg,1))
end do

    dt_ug(:,:,:) = dt_ug(:,:,:) + dt_ug_nudging(:,:,:)
    dt_vg(:,:,:) = dt_vg(:,:,:) + dt_vg_nudging(:,:,:)
...
!=================================================================================================================================
subroutine get_wind ( Time, p_half, uz, vz)
...
call interpolator( u_interp, Time, p_half, uz, trim(u_wind_file))
call interpolator( v_interp, Time, p_half, vz, trim(v_wind_file))
...
call interpolator( t_interp, Time, p_half, tm, trim(t_file))
...
!call interpolator( tau_interp, p_half, tauf, trim(equilibrium_tau_file))
call interpolator( tau_interp, Time, p_half, taum, trim(tau_file))
