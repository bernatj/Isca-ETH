...
real    :: hc=1.00
real    :: alpha=1.00           !factor to multiply the latent heat
logical :: do_alpha=.false.     !option to use the alpha factorto multiply the latent heat
logical :: do_evap=.false.
logical :: do_simple =.false.

namelist /lscale_cond_nml/  hc, do_evap, do_simple, do_alpha, alpha
...
! multiply by a factor alpha the latent heat
      if(do_alpha) then 
        hlcp = alpha*hlcp
      end if
