    ...
    do j=1,size(latb)-1
       ...
       if( abs(lat) .le. 1.0 ) then
 	     ...
 	     if (warmpool_localization_choice == 1 ) then
               flux(i,j) = flux(i,j) &
                    &+ (1.-lat**2.)*warmpool_amp*cos(warmpool_k*(lon-piphase))  !modified by cig, nov 15 2017
	     elseif (warmpool_localization_choice == 2 ) then  !assumes k=2
		if( lon .ge. (warmpool_phase - 45)*pi/180. .AND. lon .lt. (warmpool_phase + 135)*pi/180.) then !modified by cig, nov 15 2017
                  flux(i,j) = flux(i,j) &
