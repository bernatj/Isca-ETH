...
baseexp = Experiment('fixed_sst_clim_strat_relax', overwrite_data=False)
...
baseexp.inputfiles = [os.path.join(GFDL_BASE,'input/land_masks/era_land_t42.nc'),os.path.join(GFDL_BASE,'input/rrtm_input_files/ozone_null.nc'),
                      os.path.join(GFDL_BASE,'input/temp-equilibrium/temp.nc'),os.path.join(GFDL_BASE,'input/rrtm_input_files/co2_null.nc'),  
                      os.path.join(GFDL_BASE,'input/temp-equilibrium/tau.nc'),os.path.join(GFDL_BASE,'input/sst/sst_clim_ersst.nc'),
...
diag.add_field('hs_forcing', 'tau',time_avg=True)
...
baseexp.namelist['hs_forcing_nml'] = f90nml.Namelist({
   ...
   # 'equilibrium_t_file' : 'temp',
    ...
    'equilibrium_tau_option' : 'from_file',
    'equilibrium_t_option' : 'from_file',
...
baseexp.namelist['spectral_dynamics_nml']['valid_range_T'] = [50, 500]
...
#remove ozone, water vapor and co2 above 250hPa: 
...
baseexp.namelist['rrtm_radiation_nml']['co2_file'] = 'co2_null' #Tell model name of co2 input file
baseexp.namelist['rrtm_radiation_nml']['co2_variable_name'] = 'co2_null'

baseexp.namelist['rrtm_radiation_nml']['do_fixed_water'] = True   #Fix water above the tropopause
baseexp.namelist['rrtm_radiation_nml']['fixed_water'] = 0.0 #what value? [kg/kg]
baseexp.namelist['rrtm_radiation_nml']['fixed_water_pres'] = 1000.e02 #above 250hPa

#co2 netdbaseexp.namelist['spectral_init_cond_nml']['initial_temperature'] = 270 #Lower than normal initial temperature
baseexp.namelist['spectral_dynamics_nml']['initial_sphum'] = 0.0 #No initial specific humidity


#Lets do a run!
baseexp.runmonth(16, use_restart=True, num_cores=32, overwrite_data=True, light=False)
#for i in range(2,60):
