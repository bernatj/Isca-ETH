...
baseexp = Experiment('fixed_sst_clim_t85', overwrite_data=True)

#Add any input files that are necessary for a particular experiment.
baseexp.inputfiles = [os.path.join(GFDL_BASE,'input/land_masks/era_land_t85.nc'),os.path.join(GFDL_BASE,'input/rrtm_input_files/ozone_era.nc'),
...
baseexp.namelist['idealized_moist_phys_nml']['land_file_name'] = 'INPUT/era_land_t85.nc' #Tell model where to find input file

baseexp.namelist['spectral_init_cond_nml']['topog_file_name'] = 'era_land_t85.nc' #Name of land input file, which will also contain topography if generated using Isca's `land_file_generator_fn.py' routine.
...
baseexp.namelist['rrtm_radiation_nml']['dt_rad'] = 2160  #Use 4320 as RRTM timestep
...
#Lets do a run!
baseexp.runmonth(1, use_restart=False, num_cores=64, overwrite_data=True, light=False)
for i in range(2,51):
	#diag.add_file('atmos_daily_%2d' % i, 1, 'days', time_units='days')
	baseexp.runmonth(i, num_cores=64, use_restart=True, light=False)
