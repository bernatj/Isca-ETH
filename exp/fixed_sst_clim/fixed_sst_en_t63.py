...
baseexp = Experiment('fixed_sst_en_regx4_t63', overwrite_data=True)

#Add any input files that are necessary for a particular experiment.
baseexp.inputfiles = [os.path.join(GFDL_BASE,'input/land_masks/era_land_t63.nc'),os.path.join(GFDL_BASE,'input/rrtm_input_files/ozone_era.nc'),
                      os.path.join(GFDL_BASE,'input/rrtm_input_files/h2o_era.nc'),os.path.join(GFDL_BASE,'input/sst/sst_en_regx4_ersst4.nc'),
                       os.path.join(GFDL_BASE,'input/seaice/siconc_clim_eraint.nc'),os.path.join(GFDL_BASE,'input/nudging/u_clim_isca.nc'),
                       os.path.join(GFDL_BASE,'input/nudging/tau_1.nc'),os.path.join(GFDL_BASE,'input/nudging/v_clim_isca.nc'),
                       os.path.join(GFDL_BASE,'input/nudging/t_clim_isca.nc')]
...
baseexp.namelist['spectral_dynamics_nml']['num_fourier'] = 63 #Number of Fourier modes
baseexp.namelist['spectral_dynamics_nml']['num_spherical'] = 64 #Number of spherical harmonics in triangular truncation
baseexp.namelist['spectral_dynamics_nml']['lon_max'] = 192 #Lon grid points
baseexp.namelist['spectral_dynamics_nml']['lat_max'] = 96 #Lat grid points
baseexp.namelist['spectral_dynamics_nml']['fourier_inc'] = 1
...
baseexp.namelist['idealized_moist_phys_nml']['land_file_name'] = 'INPUT/era_land_t63.nc' #Tell model where to find input file

baseexp.namelist['spectral_init_cond_nml']['topog_file_name'] = 'era_land_t63.nc' #Name of land input file, which will also contain topography if generated using Isca's `land_file_generator_fn.py' routine.
...
baseexp.namelist['mixed_layer_nml']['sst_file'] = 'sst_en_regx4_ersst4' #Set name of sst input file
...
baseexp.namelist['rrtm_radiation_nml']['dt_rad'] = 2700  #Use 4320 as RRTM timestep
...
baseexp.namelist['rrtm_radiation_nml']['fixed_water_lat'] = 40 #north of 40
...
#Lets do a run!
#baseexp.runmonth(10, use_restart=False, num_cores=48, overwrite_data=True, light=False)
for i in range(25,111):
	#diag.add_file('atmos_daily_%2d' % i, 1, 'days', time_units='days')
	baseexp.runmonth(i, num_cores=48, use_restart=True, light=False)
