...
baseexp = Experiment('fixed_sst_clim_ml_10m', overwrite_data=False)

#Add any input files that are necessary for a particular experiment.
baseexp.inputfiles = [os.path.join(GFDL_BASE,'input/land_masks/era_land_t42.nc'),os.path.join(GFDL_BASE,'input/rrtm_input_files/ozone.nc'),
...
baseexp.namelist['mixed_layer_nml']['depth'] = 10. #Mixed layer depth (before 20)
baseexp.namelist['mixed_layer_nml']['land_option'] = 'input' #Tell mixed layer to get land mask from input file
baseexp.namelist['mixed_layer_nml']['land_h_capacity_prefactor'] = 0.2 #What factor to multiply mixed-layer depth by over land. (before 0.1) 
...
#Lets do a run!
#baseexp.runmonth(1, use_restart=False, num_cores=16, overwrite_data=True, light=False)
for i in range(40,51):
	#diag.add_file('atmos_daily_%2d' % i, 1, 'days', time_units='days')
	baseexp.runmonth(i, num_cores=16, use_restart=True, overwrite_data=True, light=False)
