...
1. Install the upstream Isca prerequisites (GFDL dycore, Python modules, MPI toolchain) following the original documentation from ExeClim.
2. Set the `GFDL_BASE` environment variable so the scripts can find compiled binaries and shared input datasets.
3. From the repository root, run the main driver script in `exp/fixed_sst_clim/fixed_sst_clim.py`. This script compiles the model if needed, prepares the run directory, and iterates through the experiment months.

Adjustments to SST patterns, diagnostics, or physics options can be made directly inside the files located in the `exp/fixed_sst_clim` folder.
...
I’m happy to discuss any of this in more detail—feel free to reach out to schedule a quick call (Monday afternoon or Tuesday next week usually works best).
